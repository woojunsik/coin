...
export default function BithumbRegisterForm({ onSuccess }){
  ...
  const handleRegister = async () => {
    ...
    setLoading(true);
    try {
      const res = await axios.post('/api/bithumb/balance', {
  ...
  return (
    <div className="rounded-xl border border-gray-200 bg-white shadow-sm p-6 space-y-4 w-full max-w-md">
      <h4 className="text-lg font-semibold text-gray-800">빗썸 API 키 등록</h4>
      <input
        type="text"
