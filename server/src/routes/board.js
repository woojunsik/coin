...
const getUserInfo = async (id) => {
  try {
    if (!mongoose.Types.ObjectId.isValid(id)) {
      const user = await User.findOne({ nickname: id }).select(
        "nickname profile"
      );
      return user
    ...
    const processed = await Promise.all(
      boards.map(async (board) => {
        ...
        const processedComments = await Promise.all(
          (board.comments || []).map(async (comment) => {
...
router.get("/:id", async (req, res) => {
  try {
    ...
    post.liked = userId
      ? post.likedUsers?.some((uid) => uid.toString() === userId)
      : false;
    ...
    const processedComments = await Promise.all(
      (post.comments || []).map(async (comment) => {
        const commentWriter = await getUserInfo(comment.writer);
        const liked = userId
          ? (comment.likedUsers || []).some((uid) => uid.toString() === userId)
          : false;
...
router.post("/:id/like", verifyToken, async (req, res) => {
  try {
    ...
    await post.save();
    res.json({ liked: isLike, likes: post.likes });
  } catch (err) {
...
router.get("/liked/:userId", async (req, res) => {
  try {
    ...
    const posts = await Board.find({ likedUsers: userId }).sort({
      createdAt: -1,
    });
    res.json(posts);
  } catch (err) {
